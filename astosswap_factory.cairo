...
func createPair{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (token0, token1) = AstosSwapLibrary.sortTokens(tokenA, tokenB)

    # Pair exists
    with_attr error_message("AstosSwap: PE"):
    ...
    IAstosSwapPair.initialize(contract_address=newPair, token0=token0, token1=token1)
