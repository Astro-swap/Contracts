...
from interfaces.IAstosSwapFactory import IAstosSwapFactory
from interfaces.IAstosSwapPair import IAstosSwapPair
from libraries.AstosSwap_library import AstosSwapLibrary
...
func quote{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amountB : Uint256) = AstosSwapLibrary.quote(amountA, reserveA, reserveB)
...
func getAmountOut{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amountOut : Uint256) = AstosSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut)
...
func getAmountIn{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amountIn : Uint256) = AstosSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut)
...
func getAmountsOut{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amounts_len, amounts : Uint256*) = AstosSwapLibrary.getAmountsOut(
...
func getAmountsIn{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amounts_len, amounts : Uint256*) = AstosSwapLibrary.getAmountsIn(
...
func addLiquidity{
    ...
    let (pair) = AstosSwapLibrary.pairFor(factory, pairClass, tokenA, tokenB)
    ...
    let (liquidity : Uint256) = IAstosSwapPair.mint(contract_address=pair, to=to)
...
func removeLiquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("AstosSwapRouter: IAA"):
        let (lt0) = uint256_lt(amountA, amountAMin)
        assert lt0 = FALSE
    end

    # Insufficient_b_amount
    with_attr error_message("AstosSwapRouter: IBA"):
...
func swapExactTokensForTokens{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("AstosSwapRouter: IOA"):
        # amounts[amounts_len - 1] >= amountOutMin
        let (le) = uint256_le(amountOutMin, amounts[amounts_len - 1])
        assert le = TRUE
    end

    let (pair) = AstosSwapLibrary.pairFor(factory, pairClass, path[0], path[1])
...
func swapTokensForExactTokens{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("AstosSwapRouter: EIA"):
...
func _ensure{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(deadline : felt):
    ...
    # Expired
    with_attr error_message("AstosSwapRouter: E"):
...
func _addLiquidity{
    ...
    let (pair) = IAstosSwapFactory.getPair(contract_address=factory, tokenA=tokenA, tokenB=tokenB)
    if pair == 0:
        IAstosSwapFactory.createPair(contract_address=factory, tokenA=tokenA, tokenB=tokenB)
    end

    let (reserveA, reserveB) = AstosSwapLibrary.getReserves(factory, pairClass, tokenA, tokenB)
    ...
    if b0 == TRUE:
        ...
        let (amountBOptimal) = AstosSwapLibrary.quote(amountADesired, reserveA, reserveB)
        ...
        if le1 == TRUE:
            ...
            let (amountAOptimal) = AstosSwapLibrary.quote(amountBDesired, reserveB, reserveA)
...
func _removeLiquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    IAstosSwapPair.transferFrom(contract_address=pair, sender=caller, recipient=pair, amount=liquidity)

    let (amount0 : Uint256, amount1 : Uint256) = IAstosSwapPair.burn(contract_address=pair, to=to)

    let (token0, _) = AstosSwapLibrary.sortTokens(tokenA, tokenB)
...
func _swap{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (token0, _) = AstosSwapLibrary.sortTokens(input, output)
    ...
    let (pair) = AstosSwapLibrary.pairFor(factory, pairClass, input, output)
    IAstosSwapPair.swap(contract_address=pair, amount0Out=amount0Out, amount1Out=amount1Out, to=to)
...
func _swap_to{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    # If i < path.length - 2
    if r_le == FALSE:
        let (pair) = AstosSwapLibrary.pairFor(factory, pairClass, output, path[i + 2])
        return (to=pair)
    else:
        return (to=_to)
    end
...
func _swapSupportingFeeOnTransferTokens{
    ...
    let (reserve0, reserve1, _) = IAstosSwapPair.getReserves(contract_address=pair)
    ...
    let (amountOutput) = AstosSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput)
